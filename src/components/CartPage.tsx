  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const templateParams = {
        to_email: 'oussamasassour@gmail.com',
        ...
        order_items: state.items.map(item => 
          `${item.name} - Quantity: ${item.quantity} - Price: $${item.price} - Subtotal: $${(item.price * item.quantity).toFixed(2)}`
        ).join('\n'),
        order_total: `$${state.total.toFixed(2)}`,
